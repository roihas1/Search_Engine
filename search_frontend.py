...
def search():
    ...
    body_candidates = my_get_topN_score_for_queries(query_tokens, app.index_body, 1500)  # list 100 top scores by tf-idf
    final_score = body_candidates
    N = 200
    if len(q) <= 10 or len(final_score) == 0:
        title_score = get_resault_for_query_binary_with_counter(app.index_title, original_query_token, "title", 1500)
        ...
        final_score = merge_results(title_score, body_candidates, len(original_query_token), title_weight, body_weight,
                                    N)
