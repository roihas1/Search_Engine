...
class MultiFileReader:
    ...
    def read(self, locs, n_bytes,typeIndex):
        ...
        dir_name = "postings_"+typeIndex+"/"
        for f_name, offset in locs:
            #f_name = "postings_title/"+f_name
...
class InvertedIndex:  
    ...
    def posting_lists_iter_for_anchor(self,query,typeIndex):
        ...
        with closing(MultiFileReader()) as reader:
            for w in np.unique(query):
                if w not in self.posting_locs.keys():
                    continue
                locs = self.posting_locs[w]
                b = reader.read([locs[0]], self.df[w] * TUPLE_SIZE, typeIndex)
                posting_list = []
    ...
    def posting_lists_iter(self,query,typeIndex):
        """ A generator that reads one posting list from disk and yields 
            a (word:str, [(doc_id:int, tf:int), ...]) tuple.
        """
        dic={}
